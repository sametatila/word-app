...
/* ----------- 330. CEFR ROZETININ TONLARI: BEYAZ YAZIYLA OLCULMUS DEGERLER
 *
 * Rozet DOLU zemin + beyaz yazi ve yaninda rengi aciklayan bir etiket yok,
 * yani renk tek tasiyici: KATI esik (AA 4.5, yazi 15 px). Web bunu olcup
 * duzeltmisti (`components/level-badge` `TONE`: ailelerin 600'u, B2 700'u).
 * Android ise ROL renklerini kullaniyordu (`colors.success`, `colors.info`,
 * `colors.primary`…) ve o renkler kart ustunde okunacak yazi icin degil,
 * isaret icin ayarli. Olcum (acik tema, beyaz yazi): A1 3.55, A2 3.61,
 * B1 4.91, B2 2.77, C1 4.30 - besten dordu esigi tutmuyor, B2 buyuk yazi
 * esigi 3.0'i bile tutmuyordu.
 *
 * Android tonlari artik `theme/colors` `LEVEL_TONE`da ve TEMAYA DUYARLI
 * DEGIL: rozet kendi zeminini getiriyor, temanin yuzeyiyle isi yok. `onFill`
 * kullanilmiyor - o jeton koyu temada koyu murekkebe donuyor ve bu zeminlerde
 * okunmaz.
 *
 * Olcu web tablosunu CSS'ten COZEREK karsilastiriyor (`var(--color-mint-600)`
 * -> `#237a4c`), yani iki taraf ayni degeri tasiyor mu sorusuna bakiyor;
 * kapinin icinde tonlarin bir kopyasi YOK.
 *
 * Ikinci olcu ayni bilesenin sayisi: "1234 kelime pekisti" Android'de binlik
 * ayracsiz yaziliyordu (web bastan beri `formatNumber`). */
{
  const silL = (x) => x.replace(/\/\*[\s\S]*?\*\//g, (t) => t.replace(/[^\n]/g, " ")).replace(/\/\/[^\n]*/g, "");
  const css = read("src/app/globals.css");
  const cozum = new Map([...css.matchAll(/--color-([a-z]+-\d+):\s*(#[0-9a-fA-F]{6})/g)].map((m) => [m[1], m[2].toLowerCase()]));
  const webRozet = silL(read("src/components/level-badge.tsx"));
  const webTablo = new Map(
    [...webRozet.matchAll(/^\s{2}([A-C]\d): "var\(--color-([a-z]+-\d+)\)",$/gm)].map((m) => [m[1], cozum.get(m[2]) ?? "COZULMEDI:" + m[2]]),
  );
  const mobRenk = silL(read("mobile/src/theme/colors.ts"));
  ...
  const mobGovde = (mobRenk.match(/export const LEVEL_TONE: Record<string, string> = \{([\s\S]*?)\};/) ?? ["", ""])[1];
  const mobTablo = new Map(
    [...mobGovde.matchAll(/([A-C]\d): (?:"(#[0-9a-fA-F]{6})"|orange\[(\d+)\])/g)].map((m) => [m[1], (m[2] ?? orangeTablo.get(m[3]) ?? "YOK").toLowerCase()]),
  );
  const seviyeler = ["A1", "A2", "B1", "C1", "B2"];
  sameList(
    "cefr rozetinin tonlari",
    seviyeler.map((s) => s + "=" + (mobTablo.get(s) ?? "TON YOK")),
    seviyeler.map((s) => s + "=" + (webTablo.get(s) ?? "TON YOK")),
    "mobil",
    "web (cozulmus)",
  );
  const mobRozet = silL(read("mobile/src/ui/LevelBadge.tsx"));
  sameList(
    "cefr rozetinin murekkebi ve sayisi",
    [
      "mobil murekkep=" + (/color=\{LEVEL_INK\}/.test(mobRozet) ? "beyaz (jeton)" : /colors\.onFill/.test(mobRozet) ? "TEMAYA BAGLI" : "BILINMIYOR"),
      "mobil jeton=" + ((mobRenk.match(/export const LEVEL_INK = "(#[0-9a-fA-F]{6})"/) ?? [])[1] ?? "YOK").toLowerCase(),
      "web murekkep=" + (/text-strong text-white/.test(webRozet) ? "beyaz (sinif)" : "FARKLI"),
      "mobil sayi=" + (/t\("level\.mastered_count", \{ n: formatNumber\(mastered\) \}\)/.test(mobRozet) ? "bicimli" : "HAM"),
      "web sayi=" + (/t\("level\.mastered_count", \{ n: formatNumber\(mastered, lang\) \}\)/.test(webRozet) ? "bicimli" : "HAM"),
    ],
    [
      "mobil murekkep=beyaz (jeton)",
      "mobil jeton=#ffffff",
      "web murekkep=beyaz (sinif)",
      "mobil sayi=bicimli",
      "web sayi=bicimli",
    ],
    "bulunan",
