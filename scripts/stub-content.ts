 ...
 *
 * Yalnız `conversations/*` paketi doldurulıyor: kurucunun okuduğu tek paket o.
 ...
 */
import { sourceConversationsFor } from "../src/lib/conversations/source";
import type { PackCourse } from "../src/lib/content/packs";

/** `conversations/de-a1` → { course: "de", level: "A1" } */
function parseConversationPack(pack: string): { course: PackCourse; level: string } | null {
  const m = /^conversations\/(de|en)-([abc][12])$/.exec(pack);
