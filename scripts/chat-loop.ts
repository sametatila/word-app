...
import { chatPrompt, type ChatTurn } from "../src/lib/conversations/chat";

const TURNS = Number(process.env.LOOP_TURNS) || 20;
const CONVERSATION = findConversation(process.env.LOOP_CONVERSATION || "de-a1-hallo")!;
...
async function main() {
  ...
  const history: ChatTurn[] = [{ role: "assistant", content: CONVERSATION.chat.opening }];
  const bodies: string[] = [CONVERSATION.chat.opening];
  const repeats: string[] = [];

  console.log(`${TURNS} tur · ${CONVERSATION.id} (${CONVERSATION.title})\n`);
  console.log(`  0. model   : ${CONVERSATION.chat.opening}`);
