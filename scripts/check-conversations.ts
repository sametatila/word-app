 ...
 * data/conversations-plan/SPEC.md): tip denetimi yapının doğruluğunu, bu betik
 ...
 *
 * Bilinen uyarılar `check-conversations.taban.json`da duruyor (bkz.
 ...
 * görmek için: `npm run check:conversations -- --uyarilar`.
 */
...
import { scoredSteps, type Conversation } from "../src/lib/conversations/types";
...
const repeatsOf = (l: Conversation) =>
  l.lecture
...
check("kimlikler kurallı (kurs-seviye-slug)",
  CONVERSATIONS.every(idOk),
  `(${CONVERSATIONS.filter((l) => !idOk(l)).map((l) => l.id).slice(0, 4).join(", ")})`);
...
for (const l of CONVERSATIONS) {
  ...
  */
  ...
  /*
    ...
    oynatıcı onu zaten marka rengiyle ve kalın yazıyor (conversation-player).
  ...
  */
  ...
  // Konuşma fazı
  ok(l.chat.scene.trim().length > 30, "sahne yeterince anlatılmış");
  ok(l.chat.partner.trim().length > 5, "rol tanımı var");
  ok(l.chat.opening.trim().length > 0 && l.chat.openingTr.trim().length > 0, "açılış çift dilli");
  ok(l.chat.opening.includes("?"), "açılış soruyla bitiyor");
  ...
  ok(prompt.includes(l.chat.goal), "istem konuşmanın amacını taşıyor");
...
const answers = CONVERSATIONS.flatMap((l) =>
  l.lecture
...
const targets = CONVERSATIONS.flatMap((l) =>
  l.lecture
...
const statements = CONVERSATIONS.flatMap((l) =>
  l.lecture
    .filter((s) => s.expect?.kind === "truefalse")
...
const taban = uyariTabani({
  ad: "check:conversations",
  dosya: path.join(import.meta.dirname, "check-conversations.taban.json"),
  komut: "npm run check:conversations",
  uyarilar,
});
