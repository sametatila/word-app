 ...
 *   npx tsx --tsconfig scripts/tsconfig.e2e.json scripts/tts-lesson-jobs.ts <çıktı.jsonl>
 *
 * lesson-player her adımda bir segment dizisini (`say`, ipucu `hint`, düzeltme `why` …) tek parti olarak
 ...
 * TURKISH_VOICE (Emel), hedef dil → lessonVoice (Katja/Jenny) — hepsi Defne (tr/de/en). Adımın başına eklenen
 * anlatım övgüleri (`narration` bayraklı, birleşmez) ayrı istek; onlar da listede.
 * Rol yapma (`roleplay`) Edge'de kalıyor (envanterle aynı karar); Zürih (gsw-zh) dersleri kapsam dışı.
 */
...
function emit(id: string, segs: Seg[]) {
  merge(segs).forEach((s, k) => {
    ...
    seen.add(key);
    lines.push(JSON.stringify({ id: `${id}.${k}`, field: "lesson", lang: s.lang, text: s.text, clean, long: true }));
