...
/** ünite -> o ünitenin SONUNA kadar öğretilmiş sözcükler */
const cum = new Map();
{
  let acc = new Set();
  for (let u = 1; u <= 25; u++) {
 ...
 */
function olc(ham0, unit, ekIzin = []) {
  const izin = new Set([...(cum.get(unit) || [])]);
...
module.exports = { SERBEST, havuzKok, cum, norm, parcala, türkçeMi, olc, ozet, TAKVIM, sayiMi };
