...
import { weeklyStatus } from "../src/lib/weekly";
...
import { sourceModuleContent as moduleContent } from "../src/lib/conversations/module-content-source";
...
async function main() {
  ...
  */
  ...
  console.log("\n11r) Konuşma içeriği: anlatım senaryosu ve konuşma");
  {
    ...
    check("anlatım onayla başlıyor",
      CONVERSATIONS.every((l) => l.lecture[0]?.expect?.kind === "confirm"));
    check("anlatım yeterince uzun",
      CONVERSATIONS.every((l) => l.lecture.length >= 12),
      `(en kısa: ${Math.min(...CONVERSATIONS.map((l) => l.lecture.length))})`);

    // Öğretilen her kelime anlatımda tekrar ettirilmeli — listede durup sesli
    // hiç söyletilmeyen kelime öğretilmiş sayılmaz.
    const repeats = (l: (typeof CONVERSATIONS)[number]) =>
      l.lecture
        .filter((s) => s.expect?.kind === "repeat")
        .map((s) => (s.expect as { target: string }).target.toLowerCase());
    check("her kelime tekrar ettiriliyor",
      CONVERSATIONS.every((l) => l.vocab.every((v) => repeats(l).some((t) => t.includes(v.de.toLowerCase())))),
    ...
    const produces = CONVERSATIONS.flatMap((l) =>
    ...
    const tfs = CONVERSATIONS.flatMap((l) =>
  ...
  console.log("\n11r1) Konuşma ilerlemesi ve tekrar merdiveni");
  {
    ...
    const failed = await recordConversation(USER, conversation, 0, true, conversationDay);
  ...
  console.log("\n11r2) Konuşma istemi konuşmaya bağlı");
  {
    ...
    check("geçmiş sınırı en uzun konuşmayı taşıyor", MAX_HISTORY >= conversation.chat.minTurns * 2 + 1, `(${MAX_HISTORY})`);
  }
  ...
  */
  ...
  console.log("\n38) Haftalık quiz — durum okuması");
  /*
    ESKİ BLOK KALDIRILDI. Burada `buildWeeklyExam` ve `finishWeekly` sınanıyordu;
    ikisi de silindi çünkü eski yazma yolu istemcinin bildirdiği doğruluğa
    güveniyordu. Quiz'in çekirdeği (seçim, anadil çözümü, puanlama, sızıntı)
    veritabanısız sınanıyor: `npm run test:quiz` — 45 doğrulama.

    Burada kalan tek şey bu paketin gerçekten ölçebileceği şey: `exams`
    tablosuna yazılan satırın `weeklyStatus` tarafından okunabilmesi. O satırı
    bugün `/api/quiz` yazıyor ve gelişim grafiği ile başarımlar onu okuyor.
  */
  await reset();
  await ensureProfile(USER, "E2E");
  let ws = await weeklyStatus(USER, monday);
  check("hafta Pazartesi, henüz yapılmadı", ws.week === monday && !ws.done);
  ...
  */
  ...
  check("başka modül başka kâğıt (A2.5)", pmB.cover?.code === "A2.5" && pmB.sections.reading[0].id === "r:A2.5");

  const sc1 = scoreSections({ sections: [{ id: "vocab", correct: 10, total: 12 }, { id: "grammar", correct: 9, total: 12 }, { id: "reading", correct: 5, total: 6 }, { id: "listening", correct: 4, total: 6 }, { id: "writing", correct: 0, total: 1 }], writingScore: 75, seconds: 100 }, "level");
  check("geçme: toplam ≥70 ve her bölüm ≥50", sc1.passed && sc1.total >= 70 && sc1.sections.find((s) => s.id === "writing")?.pct === 75, JSON.stringify(sc1.total));
  check("bölüm ağırlıkları %100'e tamamlanıyor", Math.abs(sc1.sections.reduce((a, s) => a + s.weight, 0) - 100) <= 1, String(sc1.sections.reduce((a, s) => a + s.weight, 0)));
  const sc2 = scoreSections({ sections: [{ id: "vocab", correct: 12, total: 12 }, { id: "grammar", correct: 12, total: 12 }, { id: "reading", correct: 6, total: 6 }, { id: "listening", correct: 2, total: 6 }], seconds: 100 }, "level");
  check("bir bölüm <50 → toplam yüksek olsa da kalır", !sc2.passed && sc2.total >= 70);
  // Üretim ağırlığı: tek maddelik yazma bölümü madde sayısına göre %4 ederdi.
