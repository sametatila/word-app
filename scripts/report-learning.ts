 ...
 */
  ...
  // 10. Telaffuz puanı
  head("10. Telaffuz puanı", "klip · ortalama puan · 80 ve üstü · kişi");
  const pr = (await q`
  ...
  `) as Row[];
  if (n(pr[0]?.n)) console.log(`  ${pr[0].n} klip · ort ${pr[0].avg_score} · geçen ${pct(n(pr[0].passed), n(pr[0].n))} · ${pr[0].people} kişi`);
  ...
  `) as Row[];
  head("11. Tur türleri", "karışık · tek oyun · ek tur — üretim oranı düşükse tek oyun tercihi ipucu verir");
  ...
  // 12. Ekranda geçen süre — öğrenme yüzeyleri
  const ts = (await q`
    ...
    from events where name = 'time_spent' and day >= current_date - ${days}::int
      and kind in ('learn','immersion','conversation','conversation_scored','skill','exam','placement','weekly')
    group by 1 order by seconds desc
  `) as Row[];
  head("12. Öğrenme yüzeylerinde geçen süre", "ekran · toplam dakika · kalış · ortalama kalış · kişi");
  ...
  // 13. Sağlayıcı sağlığı — sessizce düşen konuşma hattı
  const ai = (await q`
  ...
  `) as Row[];
  head("13. AI / STT sağlığı", "tür · sağlayıcı · istek · başarı · ort gecikme — başarı %90 altı = zincir çalışıyor ama kota yakın");
