...
if (existsSync(`${DIR}out`))
  for (const f of readdirSync(`${DIR}out`).filter((x) => x.endsWith(".json"))) {
    ...
    for (const r of JSON.parse(readFileSync(`${DIR}out/${f}`, "utf8"))) {
      ...
      const U = (m) => warnings.push(`  [${packet}] ${r.conversation} «${r.de}» — ${m}`);
...
const real = new Set(extractVocab("patterns").map((r) => `${r.conversation} ${r.de}`));
...
if (ARG === "all") {
  ...
  const missing = rows.filter((r) => !written.has(`${r.conversation} ${r.de}`)).length;
