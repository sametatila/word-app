/**
 * Paketi yazar: `node data/conversations/vocab/write.mjs <paket> < en-satirlari.txt`
 *
 * Girdi, paketin SIRASIYLA bire bir eşleşen İngilizce karşılık satırlarıdır.
 * `conversation` alanı paket dosyasından kopyalanır, yazan taraf ona hiç dokunmaz.
 *
 * Neden böyle: ilk pakette `conversation` alanını Almanca kelimeden tahmin ettim ve
...
const out = src.words.map((w, i) => ({ conversation: w.conversation, de: w.de, en: lines[i] }));
mkdirSync(`${DIR}out`, { recursive: true });
writeFileSync(
  `${DIR}out/${packet}.json`,
  `[\n${out
    .map((r) => ` { "conversation": ${JSON.stringify(r.conversation)}, "de": ${JSON.stringify(r.de)}, "en": ${JSON.stringify(r.en)} }`)
    .join(",\n")}\n]\n`,
);
