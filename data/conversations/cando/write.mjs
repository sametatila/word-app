/**
 * Paketi yazar: `node data/conversations/cando/write.mjs <paket> < satirlar.txt`
 *
 * Anahtar (`id`) paket dosyasından kopyalanıyor; yazan taraf yalnız metni
 * veriyor. Sözlükçe hattında anahtarı elle yazınca elli maddenin otuz
 * altısı tutmamıştı.
 */
...
const out = src.words.map((w, i) => ({ id: w.id, en: lines[i] }));
mkdirSync(`${DIR}out`, { recursive: true });
writeFileSync(
  `${DIR}out/${packet}.json`,
  `[\n${out.map((r) => ` { "id": ${JSON.stringify(r.id)}, "en": ${JSON.stringify(r.en)} }`).join(",\n")}\n]\n`,
);
