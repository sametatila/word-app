/**
 * Anlatım metnini paketler: `node data/conversations/lecture/make.mjs`
 *
...
export function extractLecture() {
  /** dize → { n, conversations:Set, ctx:Set<adımın tamamı>, de:Set<Almanca> } */
  const rows = new Map();
  for (const f of readdirSync(SRC).filter((x) => /^de-.*\.ts$/.test(x)).sort()) {
    ...
    for (const m of src.matchAll(/id:\s*"(de-[^"]+)"|(?:say|hint|why):\s*\[([\s\S]*?)\n\s*\]/g)) {
      ...
      for (let i = 0; i < step.length; i++) {
        ...
        const t = step[i].s;
        const r = rows.get(t) ?? { tr: t, n: 0, conversations: new Set(), ctx: new Set(), de: new Set() };
        r.n++;
        if (conversation) r.conversations.add(conversation);
        r.ctx.add(
  ...
  for (const r of rows.values()) {
    ...
    if (isGloss && words.length > 1) {
      for (const de of words)
        out.push({ tr: r.tr, de, n: 1, ctx: [], conversations: [...r.conversations].slice(0, 3) });
    } else {
      out.push({
        tr: r.tr,
        n: r.n,
        ctx: [...r.ctx].slice(0, 3),
        conversations: [...r.conversations].slice(0, 3),
      });
    }
  }
