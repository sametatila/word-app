...
const DIR = `${ROOT}data/conversations/vocab`;
...
if (existsSync(`${DIR}/out`))
  for (const f of readdirSync(`${DIR}/out`).filter((x) => x.endsWith(".json"))) {
    ...
    for (const r of JSON.parse(readFileSync(`${DIR}/out/${f}`, "utf8"))) {
      const key = `${r.conversation} ${r.de}`;
      const H = (m) => errors.push(`  [${packet}] ${r.conversation} «${r.de}» — ${m}`);
      ...
      const U = (m) =>
        warnings.push({
          anahtar: `${r.conversation} «${r.de}» → ${String(r.en ?? "").trim()} | ${m}`,
          metin: `  [${packet}] ${r.conversation} «${r.de}» → ${String(r.en ?? "").trim()} — ${m}`,
        });
...
/*
  ...
  Paket dosyası `conversation` alanını zaten taşıyor, ama ilk yazımda onu
  ...
  Bu blok onu söylüyor: yazılan `(conversation, de)` çifti içerikte gerçekten var mı.
  `zurich/beispiel/apply.mjs`teki "pakete ait olmayan id" denetiminin aynısı.
*/
const real = new Set(extractVocab().map((r) => `${r.conversation} ${r.de}`));
...
if (ARG === "all") {
  taban = uyariTabani({
    ad: "check:conversations-vocab",
    dosya: `${DIR}/uyari-tabani.json`,
    komut: "npm run check:conversations-vocab",
    uyarilar: warnings,
    gerekceZorunlu: true,
  });
