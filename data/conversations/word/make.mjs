/**
 * `word()` şablonunu paketler: `node data/conversations/word/make.mjs`
 *
 ...
 *
 *   w.en  →  3.040/3.040 hazır   (`data/conversations/vocab/`, anahtar: conversation+de)
...
const SRC = new URL("../../../src/lib/conversations/content/", import.meta.url).pathname;
...
export function extractWord() {
  /** çağrılar: { conversation, n, de, tr, note } */
  const calls = [];
  ...
  */
  ...
  for (const c of calls) {
    if (!c.note) continue;
    const r = notes.get(c.note) ?? { tr: c.note, kind: "note", n: 0, ctx: [], conversations: [] };
    r.n++;
    if (r.ctx.length < 3) r.ctx.push(`[${c.de}] '${c.tr}' demek. «${c.note}»`);
    if (r.conversations.length < 3) r.conversations.push(c.conversation);
    notes.set(c.note, r);
  }
  ...
  for (const c of calls) {
    ...
    const r = ord.get(t) ?? { tr: t, kind: "ordinal", n: 0, ctx: [], conversations: [] };
    r.n++;
