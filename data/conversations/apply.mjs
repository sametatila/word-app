/**
 * Yazılan İngilizceyi çalışma anına bağlar: `node data/conversations/apply.mjs`
 *
 ...
 * (`src/lib/conversations/native.ts`) o sözlükten okuyor.
 *
...
const OUT = new URL("../../src/lib/conversations/generated/", import.meta.url).pathname;
...
if (!existsSync(derived))
  throw new Error(
    "data/conversations/vocab/derived.json yok — `node data/conversations/vocab/triage.mjs` çalıştırılmalı " +
      "(npm run conversations:apply bunu zaten yapıyor)",
  );
...
if (existsSync(swapFile))
  for (const r of JSON.parse(readFileSync(swapFile, "utf8"))) {
    for (const [from, to] of r.de) swap[r.conversation + SEP + from] = to;
