/**
 * Seviye sayacı: `node data/conversations/chat/level.mjs`
 *
...
const rows = extractChat();
const done = new Set();
for (const f of readdirSync(`${DIR}out`).filter((x) => x.endsWith(".json")))
  for (const r of JSON.parse(readFileSync(`${DIR}out/${f}`, "utf8"))) done.add(r.conversation);

const lvl = (id) => (id.match(/^de-([a-c][12])-/)?.[1] ?? "?").toUpperCase();
const t = {};
for (const r of rows) {
  const L = lvl(r.conversation);
  t[L] ??= { all: 0, done: 0 };
  t[L].all++;
  if (done.has(r.conversation)) t[L].done++;
}
console.log(
  Object.entries(t)
...
const src = new Map(rows.map((r) => [r.conversation, r]));
let q = 0, d = 0;
for (const l of done) {
