...
const SRC = new URL("../../../src/lib/conversations/content/scripts-a1.ts", import.meta.url).pathname;
...
export function extractScript() {
  ...
  /** dize → { tr, kind, n, ctx, conversations } */
  const rows = new Map();
  let conversation = null;
  let ask = null;

  const add = (text, kind) => {
    const r = rows.get(text) ?? { tr: text, kind, n: 0, ctx: [], conversations: [] };
    r.n++;
    ...
    if (conversation && !r.conversations.includes(conversation) && r.conversations.length < 3) r.conversations.push(conversation);
    rows.set(text, r);
  };
