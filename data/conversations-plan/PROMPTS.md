...
İçerik, partiler hâlinde üretilir (önerilen parti: 10 konuşma — jeton dostu;
...
```
data/conversations-plan/SPEC.md dosyasını oku ve harfiyen uygula.
data/conversations-plan/topics-[a1].md dosyasından [001–010] numaralı konuları al.
Örnek olarak src/lib/conversations/content/de-a1.ts dosyasındaki konuşmaları incele —
kalite çıtası bu; ondan daha iyi yaz, daha kötü değil.

Görev: bu [10] konuyu tam konuşmalara dönüştür.
- Dosya: src/lib/conversations/content/de-[a1]-b[01].ts
  (export const de[A1]B[01]: Conversation[] — konu numarası sırasıyla)
- ✔ işaretli konular varsa: mevcut konuşmayı İLGİLİ ESKİ DOSYADAN buraya birebir
  taşı (içeriğini değiştirme), eski dosyayı sil, importları güncelle.
- src/lib/conversations/index.ts içindeki CONVERSATIONS dizisine partiyi, konu numarası
...
Ardından ürettiğin HER konuşmayı tek tek yeniden oku ve şu üç soruya göre elden
...
(3) Üretim alıştırmaları gerçekten bu konuşmanın kalıbını mı sınıyor?
...
Her seviye 10 parti × 10 konuşma: `b01` (001–010) … `b10` (091–100).
Sıra önemli: bir seviyenin partileri numara sırasıyla üretilmeli — sarmal
tekrar, önceki konuşmaların kelimelerini bilinen sayar. Seviyeler arası sıra
serbest ama A1 → C1 gitmek, örneklerin birikmesi açısından en sağlıklısı.

İlk partiler (mevcut konuşmaları taşıdıkları için özel):
...
```
data/conversations-plan/SPEC.md kriterlerine göre src/lib/conversations/content/de-[a1]-b[01..03].ts
dosyalarındaki konuşmaları denetle. Her konuşma için: Almanca doğallık, Türkçe
...
npm run check:conversations ve npx tsc --noEmit temiz çıksın. Değişen konuşmaları ve
