 ...
 */
const levelOf: (action: string, body: Record<string, unknown>) => "read" | "normal" | "sensitive" = (a) => (a === "check_document" ? "read" : "normal");
...
export async function POST(req: Request) {
  ...
  if (writer && res.status < 400) {
    ...
    void logAdminAction(writer, `legal.${action || "save"}`, target == null ? null : String(target), {
