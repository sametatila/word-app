 ...
 * Görsel parçalar `_ui/ui` içinde, bütün yönetim sayfalarıyla ortak.
 */
...
const TABS = ["Genel bakış", "Sunucu", "Deneyim", "Öğrenme", "Büyüme", "Olaylar"] as const;
...
export function AdminDashboard({ data: d, server: s, coverage: c, openReports, revenue: r, trends }: { data: AdminData; server: ServerMetrics; coverage: Coverage; openReports: number; revenue: Revenue; trends: TrendMetric[] }) {
  ...
  /* ALARM ŞERİDİ. Sessiz kırılmalar: koşmayan zamanlanmış iş, eskiyen yedek,
     çökmüş servis. Genel bakışın EN ÜSTÜNDE: eskiden sayıların altında
     kalıyordu ve ekran kaydırılmadan görünmüyordu. */
  const alarms: string[] = [];
  ...
  if (api5xx > 0) alarms.push(`Bugün API ${api5xx} kez 5xx döndü (Sunucu › İstek sağlığı)`);
  ...
  return (
    <div className="space-y-5">
      ...
      <div role="tablist" aria-label="Bölümler" className="-mx-1 flex gap-1 overflow-x-auto border-b px-1" style={{ borderColor: "var(--border)" }}>
        {TABS.map((t) => (
          <button key={t} type="button" role="tab" aria-selected={tab === t} onClick={() => setTab(t)}
            ...
            {t}
            {t === "Genel bakış" && alarms.length ? <span className="ml-1.5 text-caption" style={{ color: TONE.bad }}>({alarms.length})</span> : null}
          </button>
        ))}
      </div>
      ...
      {tab === "Genel bakış" && (
        <div className="space-y-5">
          {alarms.length ? (
            <Notice tone="bad" title={`Dikkat: ${alarms.length} sorun`}>
              <ul className="list-disc pl-5">{alarms.map((a) => <li key={a}>{a}</li>)}</ul>
            </Notice>
          ) : (
            <Notice tone="ok">Alarm yok: zamanlanmış işler, yedekler, servisler ve sertifika yolunda.</Notice>
          )}
      ...
      {tab === "Olaylar" && (
        <PanelGrid>
          <Panel title="İstemci hataları (30g)" hint="Yakalanmamış hata — ekrana göre. Mobil + web." actions={<a href="/admin/errors" className={BTN.small}>Hata grupları</a>}>
