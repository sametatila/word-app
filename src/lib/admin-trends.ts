...
export async function weeklyTrends(): Promise<{ metrics: TrendMetric[]; issues: QueryIssue[] }> {
  ...
  const [r] = await rows(sql`
    ...
    select
      ...
      (select count(*) from events, w where name = 'lesson_finish' and day >= w.a and day < w.b)::int lessons_c,
      (select count(*) from events, w where name = 'lesson_finish' and day >= w.pa and day < w.pb)::int lessons_p,
