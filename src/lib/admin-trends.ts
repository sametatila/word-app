...
export async function weeklyTrends(): Promise<{ metrics: TrendMetric[]; issues: QueryIssue[] }> {
  ...
  const [r] = await rows(sql`
    ...
    select
      ...
      (select count(*) from events, w where name = 'conversation_finish' and day >= w.a and day < w.b)::int lessons_c,
      (select count(*) from events, w where name = 'conversation_finish' and day >= w.pa and day < w.pb)::int lessons_p,
  ...
  return {
    ...
    metrics: [
      ...
      { key: "lessons", label: "Biten ders", current: n("lessons_c"), previous: n("lessons_p"), good: "up" },
