...
import { sourceLessonsFor } from "./source";
...
function inLevel(course: string, level: string) {
  return sourceLessonsFor(course).filter((l) => l.level === level);
}
...
export function sourceModuleLessons(course: string, level: string, index: number) {
...
export function sourceModuleContent(course: string, level: string, index: number): ModuleContent {
  return buildModuleContent(course, level, index, sourceModuleLessons(course, level, index));
}
