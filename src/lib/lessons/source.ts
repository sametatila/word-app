 ...
 * doğrulama betikleri (`check:lessons`, `check:pairs`, `check:exams` …),
 * mobil tohum dökümü (`dump:lessons`) ve yayın (`content:publish`). Yani
 * kaynağı okuyan her şey KAPININ ÖNÜNDE duruyor.
 *
 * SUNUCU SAYFALARI BURAYI OKUMUYOR: web içeriği yayın hattından alıyor
 * (`lib/lessons/index`, `lib/content/serve`). Ayrım bilinçli — bu dosyayı bir
 ...
 * `lib/lessons/index`teki async sürümleri. İkisi aynı içeriği veriyor: biri
 * kaynaktan, öteki o kaynaktan üretilmiş yayından.
 */
export function sourceLessonsFor(course: string): Lesson[] {
  const order = ["A1", "A2", "B1", "B2", "C1"];
  return LESSONS.filter((l) => l.course === course).sort(
    (a, b) => order.indexOf(a.level) - order.indexOf(b.level),
  );
}

export function sourceFindLesson(id: string): Lesson | undefined {
