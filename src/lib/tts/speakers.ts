...
/**
 * Konuşmacı etiketinden ses dağıtımı.
 *
...
/**
 * Rol tablolarının ÇİFTLERİ — kapı bunları tek tek doğruluyor.
 *
 * Sebebi somut: `NEUTRAL` listesine İngilizce diye yazılan "Student" aynı
 * yazımdaki Almanca eril adı GÖLGELİYORDU (`genderOf` önce `NEUTRAL`a bakıyor)
 * ve tablo asimetrik hâle geliyordu — "Studentin" kadın, "Student" cinsiyetsiz.
 * Böyle bir gölgeleme tip hatası vermiyor ve ancak yanlış sesi duyan biri
 * şikâyet edince görülüyor. Kapı artık her eril kökün eril, her dişil biçimin
 * dişil çözüldüğünü ölçüyor.
 */
export function rolePairs(): { male: string; female: string | null }[] {
  const out: { male: string; female: string | null }[] = [];
  const femOf = (base: string) => (base.endsWith("e") ? `${base.slice(0, -1)}in` : `${base}in`);
  for (const [base, fem] of Object.entries(ROLE_IRREGULAR)) {
    if (fem === null) continue;
    out.push({ male: base, female: fem });
  }
  for (const base of ROLE_REGULAR) out.push({ male: base, female: femOf(base) });
  return out;
}
