 ...
 *
 * NEDEN SUNUCU DAYATIYOR. Katalog istemcide; düzenleme ekranı kilitliyi gri
 * gösterse bile uca doğrudan istek atılabilir. Kilit yalnız arayüzde dursaydı
 * kilit olmazdı — aynı ilke `api/stt` kapısında da yazılı.
 *
 ...
 *
 * Bu dosya `server-only` DEĞİL: içinde sorgu yok, yalnız eşleme var. Mobil
 * kopyası `mobile/src/lib/avatarUnlocks.ts` ve `check:parity` ikisini
 * karşılaştırıyor.
 */
