...
export const DE_A2_W01: QuizWeek = {
  ...
  items: [
    ...
    /* ── Dilbilgisi ─────────────────────────────────────────────────────── */
    {
      ...
      options: ["habe", "bin", "war", "hat"],
      answer: 0,
      why: "`treffen` Perfekt'i `haben` ile kurar; `sein` çoğunlukla yer değiştirme ve durum değişimi fiillerinde (gehen, fahren, werden) ve `sein`/`bleiben` ile kullanılır. Türkçede yardımcı fiil seçimi olmadığı için ikisi rastgele seçiliyor.",
    ...
    },
    {
      ...
      byNative: {
        en: {
          options: ["bin", "war", "habe", "hatte"],
          answer: 1,
          why: "`bin` İngilizce `been` gibi görünür ama şimdiki zamandır: `ich bin` = `I am`. Geçmiş `war` (= `was`). `habe`/`hatte` ise `krank` gibi bir sıfatla yüklem kuramaz: hasta olmak `sein` ister.",
        },
      },
    },
    {
    ...
    /* ── Bağlamda kelime ────────────────────────────────────────────────── */
    {
      ...
      options: ["seit", "vor", "von", "für"],
      answer: 1,
      why: "Geçmişteki bir anı bugünden geriye ölçerken `vor` + Dativ kullanılır: 'üç gün önce'. `seit` hâlâ süren bir durum içindir, bitmiş bir olayla gelmez; `für` bir süreyi ölçer, anı değil; `von` bir başlangıç noktası verir, bugünden geriye ölçmez. Türkçede 'önce' sona geldiği için edatın başa geldiği unutuluyor.",
      targets: ["zeitangabe.vor", "praeposition.dativ"],
      byNative: {
        en: {
          options: ["für", "vorher", "seit", "vor"],
          answer: 3,
          why: "İngilizce `ago` sona gelir, `for` ise süre bildirir. Almancada 'three days ago' `vor drei Tagen` olur; `für drei Tage` bir süre bildirir (ve Akkusativ ister). `three days before` → `vorher` aktarımı da tutmaz: `vorher` bir zarftır, `drei Tagen`in önüne gelip Dativ istemez.",
        },
      },
    },
    {
