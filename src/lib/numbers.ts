 ...
 *
 * Bu dosya mobil `mobile/src/lib/numbers.ts` ile BİREBİR aynı olmalı; ayrılırsa
 * aynı cevap iki platformda farklı puan alır. `scripts/parity-check.mjs` 16.
 * bölümü tabloları karşılaştırıyor.
 *
 * Web tarafı `lib/german-numbers.ts` olarak yalnız Almanca yapıyordu:
 * İngilizce kursta "forty-two" gibi bir hedef tanıyıcının yazdığı "42" ile
 * hiçbir zaman eşleşmiyordu, "achthundert" gibi çarpımsal Almanca bileşikler
 * de çözülmüyordu (mobilde `deScale` çözüyor).
 *
