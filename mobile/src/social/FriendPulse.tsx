    ...
    <PressableScale onPress={() => goFriends(nav, "friends")} style={{ marginBottom: spacing.xl }}>
      <Card padded style={{ flexDirection: "row", alignItems: "center", gap: spacing.md, borderColor: invited ? colors.info : colors.primary, borderWidth: 1.5 }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {invited ? (
            <Text variant="caption" color={colors.textMuted}>{t("friendpulse.target", { xp: formatNumber(q.targetXp), status: q.invitedByMe ? t("friendpulse.awaiting") : t("friendpulse.accept") })}</Text>
          ) : (
            <View style={{ marginTop: 6 }}>
              <Bar pct={q.pct} tint={colors.primary} />
              <Text variant="micro" color={colors.textMuted} style={{ marginTop: 3 }}>{t("friendpulse.progress", { current: formatNumber(q.totalXp), target: formatNumber(q.targetXp), n: q.daysLeft })}</Text>
            </View>
          )}
        </View>
