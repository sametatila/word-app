/**
 * Parola ölçütü — `src/lib/auth/password-policy.ts`in MOBİL KOPYASI.
 *
 * NEDEN KOPYA: web ve mobil ayrı paketler, ortak modül yok. Kural SUNUCUDA
 * uygulanıyor (better-auth kancası); buradaki kopya yalnız kullanıcı yazarken
 * ANINDA geri bildirim vermek için. İkisi ayrışırsa güvenlik açığı doğmaz ama
 * kötü bir sürpriz doğar: alan yeşil görünür, gönderince sunucu reddeder.
 * `npm run check:parity` en azından asgari uzunluğun iki tarafta aynı kaldığını
 * ölçüyor.
 *
