...
## Ek (2026-08-26): üretim tabanı

WP-14 merdiveni kurulu oturumda üretim payını garanti etmiyordu; gerçek hesaplarda (kelimelerin tamamı fresh/shaky) pay %10–27'de kaldı. `lib/session.ts raiseProductionFloor`: oturum kurulduktan sonra sondan başa tanıma turları kelimenin gücüne uygun üretim turuna çevrilir (yeni/takılan: ipuçlu yazma ya da harf bulmacası; oturmuş/sağlam: çeviri, cümle diz, yazma); aynı kelime iki kez üretilmez, tanıtım ve eşleştirme dokunulmaz, tek oyun modunda taban yok. `PRODUCTION_FLOOR` (varsayılan 0.4; 0 kapatır). Ölçüm `MIX_ACCURACY=0.35 npm run test:mix`: taban kapalı %28, açık %45; doğru cevaplayan öğrencide %42 → %45. KPI 2 tanımı da düzeltildi: tur sayar (tanıtım hariç, eşleştirme 1 tur).
