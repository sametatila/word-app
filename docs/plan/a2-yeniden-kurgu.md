...
A2 konuşmaları, kullanıcıların A2'de öğrenmesi gereken kelimelerden kopmuş
...
| Havuzda hiç olmayan konuşma kelimesi | 72 farklı madde |
...
1. A2 konuşmalarının öğrettiği her kelime havuzun **A2 katmanından** gelsin.
...
| Ne | Nerede | Not |
|---|---|---|
| Konuşma kaynağı | `src/lib/conversations/content/de-a2-b01.ts … b10.ts` | **web ve mobil bunu paylaşır** |
| Mobil konuşma paketi | `mobile/src/data/conversations/de-a2.json` | türev; `npx tsx scripts/dump-conversations-mobile.ts de` ile üretilir |
...
1. **Konuşma id'leri SABİT.** `user_conversations` birincil anahtarı `(user_id, conversation_id)`;
   id değişirse canlı kullanıcıların ilerlemesi silinir. Konuyu ve içeriği
   değiştir, id'ye dokunma. Konuşma sayısını da değiştirme (100).
2. **Web canlı.** Konuşma kaynağı web ve mobil tarafından paylaşılıyor. İçeriği
   iyileştirmek serbest, bozmak değil. Ayrı bir kopya çıkarma.
3. **`mobile/src/data/conversations/de-a2.json` TEK SATIRDIR.** Üreticisi
   `JSON.stringify(conversations)` kullanır. Elle `null, 1` ile yazarsan 40 bin
...
- konuşmaların öğrettiği maddelerin seviye dağılımı,
- A2 katmanının kapsanma oranı (öğretilen / yalnız metinde geçen / hiç yok),
- havuzda hiç olmayan konuşma kelimeleri.
...
A1'de aritmetik temiz çıktı: 790 kelime, 100 konuşma × 8 = 800 slot.
**A2'de çıkmıyor: 1412 kelime, 800 slot.** Seçenekler:
- konuşma başına ~14 kelime (konuşma süresi ciddi uzar),
- konuşma başına 8 + sıklık önceliği (kapsama ~%57'de kalır),
- konuşma sayısını artırmak (ünite düzeni ve 25 ünite varsayımı değişir; id'ler
...
Konuşmada öğretilip havuzda olmayan 72 maddeyi çıkar, her birini sınıflandır:
...
**Mekanik dağıtım denendi ve reddedildi.** Konuşma başlığı + özet + mevcut
...
"Merhaba" konuşmasına `der Empfänger`, "Nasılsın" konuşmasına `das T-Shirt` düştü.
Kelime örtüşmesi müfredat tasarımı için yeterli sinyal değil.

Bunun yerine: her modül (10 konuşma) için
1. konuşmanın mevcut A2 kelimelerini **çıpa** say, yerinde bırak,
2. seviye dışı olanları çıkar,
3. boş slotları A2 katmanından **anlamına bakarak** doldur,
4. atamayı bir betikle doğrula: her kelime A2 mi, havuzda mı, **başka bir konuşmada
...
### Adım 5 — Konuşma içeriğini yaz
Sözlükçe değişince `lecture` adımları da değişmeli: her yeni kelimenin
öğretildiği adım, çıkan kelimenin izleri, `patterns` ve `chat` tutarlılığı.
...
Ayrıca: `npx tsc --noEmit` (kök ve `mobile/`), `npm run check:conversations`,
...
### Hedef 1 — konuşmalar havuzun A2 katmanından öğretsin

100 konuşma × 8 kelime = **800 sözlükçe yuvası** (`scripts/check-conversations.ts`,
...
|---|---|---|
| A2 konuşmalarıyla kapsanan | 115/1412 (%8,1) | **792/1416 (%55,9)** |
| Hiçbir seviyenin konuşmasında geçmeyen | — | 556/1416 → **860 kapsandı (%60,7)** |

Katman 1412'den 1416'ya çıktı: konuşmada öğretilip havuzda olmayan 72 maddenin
...
1416). Kalan 624 madde ancak konuşma sayısı ya da sözlükçe boyu değişirse
...
`scripts/check-conversations.ts` sözlükçe boyunu 5'e sabitlemişti; A1 ve A2 sekize
çıkınca doğrulama kırmızıya döndü — seviyeye bağlı hâle getirildi.
`scripts/check-content.ts` `minTurns` için 2-6 uyarıyordu, `check-conversations.ts`
...
- **Kalan 624 A2 maddesi.** Konuşma sayısı ve sözlükçe boyu sabit olduğu sürece
  öğretilemez; kararı konuşma sayısını artırmak isteyen bir sonraki oturuma ait.
...
yerdeydi**: havuzda olmayan 72 + B2 31 + C1 4. Bunlar konuşmada bir kez gösterilip
...
İkinci ölçüm: konuşma sözlükçesindeki 800 kelimenin **743'ü (%92,9)** beceri
egzersizlerinin metinlerinde en az bir kez geri dönüyor. Üç katman bağlı:
konuşma (ilk karşılaşma) → SRS (aralıklı geri çağırma) → beceri (bağlamda tekrar).

### §7'deki bir cümle yanlıştı

"Kalan 624 madde ancak konuşma sayısı ya da sözlükçe boyu değişirse öğretilebilir"
...
kelimenin konuşma başlığı olması gerekmiyor. Doğrusu: *konuşma başlığı olarak
...
| Seviye | Ünite hizalı | Egzersiz (o/d/y) | Hedef | Yetim konuşma kelimesi |
|---|---|---|---|---|
...
ama o ünitelerin konuşmalarını ölçmüyorlar. 57 yetim kelimenin **39'u tam bu altı
...
- **10 dakikalık konuşmada 8 kelime yoğun** (~75 sn/kelime, tek tekrar adımı). Bu
  ...
  sıralaması ters. Konuşma sayısı sabit olduğu sürece değiştirilemez.
- **Dilbilgisi sıralaması denetlenmedi.** Konuşma kimlikleri sabit olduğu için sıra
...
| Beceri metninde hiç geçmeyen konuşma kelimesi | 57/800 | **7/800** |
...
`npm run check:conversations` → **hata yok**, 7 uyarı (hepsi B1). Kök ve `mobile/`
