# WP-74 — İngilizce konuşma kapsamı B1–C1

**Durum:** sürüyor (B1 TAM, B2 TAM; C1 modül 1-3 TAM — 230/300 konuşma) · **Sahip:** Claude · **Açılış:** 2026-09-11

## Kusur

İngilizce kursun konuşma katmanı A2'de bitiyor. Ölçüldü (2026-09-11):
...
konuşma akışı yarıda kesiliyor. Kurs kartı da `English · CEFR A1–C1` diyor.

## Kapsam

Modül başına 10 konuşma, seviye başına 10 modül: **B1 100 + B2 100 + C1 100
= 300 konuşma**. Almanca B1'in 18 modülü buraya taşınmıyor — o genişleme
...
## Sözleşme (konuşma başına)

2026-09-11'de sahibin kararıyla İngilizce kurs Almanca kursun
sözleşmesine getirildi; yeni konuşmalar o sözleşmeyle doğar:
...
- sohbet: 6–9 tur, senaryolu yedeği ile
- `tekrar adımı payı` %60'ı geçmez, `puanlanan adım` ≥ 4

Biçim JSON (`src/lib/conversations/content/en-b1.json`), TS değil — İngilizce
katman baştan böyle kuruldu.

## Yazmak ile bağlamak ayrı iki iş

Konuşma yazmak işin yarısı. Her yeni Türkçe dize **Almanca anadil ekseninde**
de karşılık ister (Almanca anadilli öğrenci İngilizce kursu alıyor):

1. `src/lib/conversations/content/en-b1.json` — konuşma
2. `node data/conversations/prose-de/make.mjs` → yeni Türkçe dizeler `in/`'e düşer
3. elle `data/conversations/prose-de/out/` paketleri — Almanca karşılıklar
4. `npm run check:native-de` → "konuşma 300/300" demeli
5. `npm run dump:native` → `mobile/src/data/native/de.json`

Kapılar: `check:conversations`, `test:content`, `check:native-de`, `test:gloss`.
...
**B1 modül 1 — İş dünyası (2026-09-11).** On konuşma: kariyer hikâyesi,
...
kez kullanıldı; konuşma başına bir kural, tekrar yok.

İki şey konuşma yazmadan ÖNCE yapıldı ve sırası önemli: can-do ifadeleri
...
**B1 modül 2 — Ev ve kira dünyası (2026-09-11).** On konuşma: daire aramak,
...
hedefinde ya da köprüde geçiyor. `check:conversations` bunu «her kelime tekrar
...
Bu modülün konusu bir alan değil bir beceri, ve her konuşma bir bağlaç
...
önce `vocabcheck` ile aday liste ölçüldü, sonra konuşma kuruldu — ters sırayla
çalışınca konuşma bitip kapı kırılıyor.

**B1 modül 6 — Eğitim ve gelişim (2026-09-11).** On konuşma: çalışma planı, sınava
...
Burada sohbet açılışı bir kez soruyla bitmedi (`en-b1-motivation`) ve kapı
haklı olarak reddetti: açılış soru değilse öğrenci ilk turda ne söyleyeceğini
bilmiyor, konuşma tek taraflı başlıyor.

**B1 modül 7 — Fikir ve tartışma (2026-09-11).** On konuşma: fikir söylemek,
...
konuşmada sabit, o yüzden Almancanın kurtarabileceği diziliş baştan seçilmeli.

İkincisi ikon: `speak` diye bir ikon yok ve bunu `check:conversations` değil
`check:content` yakalıyor. İki kapı da çalıştırılmadan modül bitmiş sayılmaz.

**B1 modül 8 — Sağlık sistemi (2026-09-11).** On konuşma: aile hekimine
...
**B1 modül 9 — Çevre ve şehir yaşamı (2026-09-11).** On konuşma: çöpü ayırmak,
...
Bu modülde kural konuşma için uydurulmadı, metnin kendi kuralı: belediye çöp
...
Eşleşme doğal olduğu için on konuşmanın dokuzu ilk kapı denemesinde geçti.

`check-conversations.ts` bir yanlış uyarı üretiyordu: «aynı kelime iki konuşmada yeni
diye öğretilmemeli» denetimi SEVİYE anahtarlıydı, kurs anahtarlı değil.
Almanca konuşmadaki `wild` ile İngilizce konuşmadaki `wild` aynı yazılıyor, farklı
...
**B1 modül 10 — Duygular ve hayaller (2026-09-11). B1 BİTTİ.** On konuşma:
...
- 100 konuşma, 100 benzersiz kimlik
- **800 sözlükçe maddesi, 800'ü de benzersiz** — seviye içinde tek tekrar yok
- 3.913 Almanca dize (l-076…l-095); `check:native-de` konuşma 300/300
- `check:conversations` hata yok, `test:content` bütçe içinde, `test:gloss` geçti

Bir etiket tutarsızlığı kapanışta düzeltildi: bir konuşma `Gerund and
...
BİLEREK ayrı bırakıldı: onlar aynı kuralın alt başlığı değil, konuşmanın gerçekten
başka bir yüzünü öğretiyor.

**B2 modül 1 — Profesyonel iletişim (2026-09-11).** On konuşma: sabah
...
ÖLÇÜLDÜ, varsayılmadı: her konuşmanın `candoForConversation` çıktısı tek tek bakıldı.

B1'de olduğu gibi konuşma yazmadan ÖNCE dört şey yapıldı ve sırası önemli:
...
4. `prose-de/make.mjs` dosya listesine `en-b2.json`, ve `src/lib/conversations`,
   `mobile/src/data/conversations`, `check-dumps.ts` bağlantıları.

Konuşmalar eşleşme doğrulandı: beş konuşmanın beşi de hedeflenen ifadeye gidiyor
...
**B2 modül 2 — Müzakere ve şikâyet (2026-09-11).** On konuşma: iddia, sorun
...
«reimbursement» ve `perform`/«performance» geçiyor — isimleştirme konuşmasının
üç kalıbı o üç fiille kuruldu, `violate` ayrı bir tekrar hedefine kondu.

**B2 modül 3 — Edilgenin bütün hâlleri (2026-09-11).** On konuşma: aktarıldığı
...
yaptığının aynısı. Her konuşma edilgenin başka bir hâlini gösteriyor: aktarım
...
yok. Doğru sıra ters: önce boştaki kelimeler listelenir, konuşma konusu ona göre
dizilir. Tek sorguda kırk kelime çıkıyor (`scratchpad/freewords.mjs`).

**B2 modül 4 — Medya ve aktarılan söz (2026-09-11).** On konuşma: başlığı
...
**B2 modül 5 — Bilim ve teknoloji (2026-09-11).** On konuşma: bir çalışmanın
...
Son konuşma modülün yükünü taşıyor: yumuşatıcı bağlayıcılar. Bilimsel sonuç
...
konuşmak sohbetin kendi işi.

Modül 5 kapandığında B2.GR.11-20'nin onu da modül içinde tam bir kez
kullanılmış oldu (`candoForConversation` ile ölçüldü, 10/10) ve B2'nin elli
konuşmasının hepsi bir ifadeye düşüyor.

**B2 modül 6 — Toplum ve ekonomi (2026-09-11).** On konuşma: eşitsizlik raporu,
...
ayırır. Son konuşma bunu doğrudan söylüyor: yumuşatıcılar hükmü verir ama onu
...
atamıyor. Segment sırası konuşmadan geldiği için çözüm her tokene kendi
...
**B2 modül 7 — Kültür ve sanat (2026-09-11).** On konuşma: katalog künyesi,
...
uyarıdır; yarık cümle yapıtın KONUSUNU değil YAPTIĞINI başa taşır. Son konuşma
...
görünüyor, arama-değiştirme ıskalıyor, `check:conversations` ve `test:content`
...
**B2 modül 8 — Para ve kariyer stratejisi (2026-09-11).** On konuşma: maliyet
...
**B2 modül 9 — İnsan ilişkileri ve psikoloji (2026-09-11).** On konuşma:
...
konuşma: resmî yazı, dosyada yazana göre, gecikmeye yol açan, işin yürüyüşü,
...
4. Dört dosya listesi: `prose-de/make.mjs`, `src/lib/conversations/index.ts`,
   `mobile/src/data/conversations/index.ts`, `scripts/check-dumps.ts`.

Bağlantı konuşma yazılmadan ÖLÇÜLDÜ: ilk beş konuşmanın beşi de ayrı bir C1
can-do'suna düşüyor.
...
Modülün kendi ekseni KÖPRÜYÜ ATMAMAK ve ilk konuşma C1'in kendi tehlikesini
...
Üç konuşma aynı aracın üç ayrı bedelini gösteriyor. Aktarma fiili
...
**C1 modül 3 — Hukuk ve sözleşme dili (2026-09-11).** On konuşma: dilekçe
...
reddedilebilir. Aynı ilke on konuşmanın hepsinde başka bir yerden görünüyor —
...
Üç konuşma aracın kendi maliyetini taşıyor. Aktarma fiili modül 3'te sessiz
...
İki konuşma C1'in araçlarını KORUMA olarak kullanıyor. Şeffaflık konuşmasında
çekimsiz fiil bir üslup değil bir siper: «ben istiyorum» diyen ifşacı,
kurumun gözünde savı değil kendisi oluyor. Aktarma konuşmasında fiil bir
...
son olarak seçim. Kapanış konuşmasının kalıbı savı kendi yapısıyla söylüyor —
...
**WP-74 KAPANDI.** İngilizce kurs A1'den C1'e 500 konuşma; C1'in 800
sözlükçe maddesi benzersiz, Almanca eksen l-112…l-121 paketleriyle tam,
`check:native-de` konuşma 500/500.

## Tamamlanma ölçütü

- `CONVERSATIONS` içinde `course: "en"` için B1/B2/C1 100'er konuşma
