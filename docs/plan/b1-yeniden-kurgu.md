...
Ayrıca B1'in kendine özgü bir sorunu var: A1 konuşmaları seviyenin ÜSTÜNE
taşıyordu; B1 konuşmaları hem üstüne (%23 B2/C1) hem ALTINA (%24 A1/A2) taşıyor.
Yani konuşma ne öğrettiğini bilmiyor.
...
1. Konuşmaların öğrettiği her kelime havuzun **B1 katmanından** gelsin.
...
| Ne | Nerede |
|---|---|
| Konuşma kaynağı | `src/lib/conversations/content/de-b1-b01.ts … b10.ts` — **web ve mobil paylaşır** |
| Mobil konuşma paketi | `mobile/src/data/conversations/de-b1.json` — türev, `npx tsx scripts/dump-conversations-mobile.ts de` |
...
1. **Konuşma id'leri SABİT.** `user_conversations` PK'si `(user_id, conversation_id)`; id
   değişirse canlı ilerleme silinir. Mevcut 100 konuşmanın id'i de katalog sırası
   da dokunulmaz.
   **Konuşma SAYISI dondurulmuş değil (2026-09-04 kararı).** Bu maddenin ilk hâli
   "konuşma sayısını da değiştirme (100)" diyordu ve §5 Adım 2'nin sunduğu
   "konuşma ekleme" seçeneğiyle çelişiyordu. Çelişki şöyle çözüldü: id dondurması
   yerinde, **sona konuşma eklemek serbest**. Yeni konuşma sayısı **20'nin katı
   olmalı** — modül 10'arlı (`MODULE_SIZE`), ünite 4'erli (`UNIT_CONVERSATIONS`)
   dilimliyor; 20'nin katı değilse son modül ya da son ünite eksik kalır.
2. **Web canlı.** Konuşma kaynağı ortak; iyileştir, bozma, ayrı kopya çıkarma.
...
| Karar | Değer |
|---|---|
| Konuşma başına kelime | **8** (A1/A2 ile aynı biçim) |
| Alt seviye (A1/A2) tekrarı | **~%10** → konuşma başına ~1, slotun %90'ı B1 |
| Konuşma sayısı | **180** = 100 mevcut + 80 yeni |
...
oturması ürün içinde tutarlı bir vaat. Brief'in ilk hâlindeki "+60 konuşma = %70"
tahmini %10 alt tekrarı hesaba katmıyordu; tekrarla +60 konuşma yalnızca %64,6
...
ulaşıyor. Konuşmanın işi bağlamda öğretmek. %100 kapsama 260 konuşma (+160, içerik
2,6 katı) ve 358 egzersiz borcu isterdi; son %20'nin marjinal değeri düşük.
Konuşmada geçmeyecek ~487 B1 kelimesi kartla gelir.

**Yapısal engel yok (kontrol edildi):** `moduleCount = ceil(konuşma/10)`
(`module-content.ts:204`), ünite sayısı `ceil(konuşma/4)`, `moduleTheme` taşan
...
`scripts/check-conversations.ts:59` (`vocabSize` seviye başına sabit kodlu — paralel
...
Özet: 3 havuzda zaten vardı (konuşma varyant yazmış) · 3 türev · **32 gerçek B1
maddesi havuza eklendi** (id 8450–8481) · 78 konuşmadan çıkacak.

Havuzun B1 katmanı 1797 → **1829** kayıt, normalize 1783 → **1815** başlık.
Bu yüzden yukarıdaki hedef kapsama %72,7 değil **%71,4**; konuşma sayısı kararı
(180) değişmedi.

### Adım 3.5 — Mevcut 100 konuşma BİTTİ (2026-09-05)
...
| `check:conversations` B1 | 100 hata | **0** |

Kalan 7 uyarı bu işten önce de vardı ve sözlükçeyle ilgisi yok: bağlaç
konuşmalarının kalıp açıklamaları Türkçe segmentin içinde Almanca örnek cümle
taşıyor. Ayrı bir iş.

### Adım 3.6 — Genişleme iskeleti: modül 11–18 (2026-09-05)

Hedef %71,4'e 100 konuşmalık iskelet yetmiyor (%41,9 veriyor); kalan 80 konuşma
...
### Adım 3.7 — 180 konuşma BİTTİ (2026-09-05)

Genişleme tamam: 18 modül, 180 konuşma, 45 ünite. **Hedef aşıldı.**
...
| `check:conversations` | 100 B1 hatası | **0** | 0 |
...
3. *Ön kontrol kipi* (`--yeni`) — konuşma yazılmadan sözlükçeyi denetler.
   Modül 16'da `der/die Angestellte` çakışmasını on konuşma YAZILMADAN önce
   yakaladı; modül 14'te aynı sınıf hatayı ancak içerik yazıldıktan sonra
   görmüştüm.

### Adım 4 — Atamayı ELLE yap, modül modül
**Mekanik dağıtım A1'de denendi ve reddedildi:** konuşma metninden anlam torbası
kurup puanlayan betik aritmetik olarak hepsini yerleştirdi ama "Merhaba"
konuşmasına `der Empfänger`, "Nasılsın" konuşmasına `das T-Shirt` düşürdü. Kelime
...
1. **Taşıyıcı kelimeyi çıkarma.** Bir kelime sözlükçede duruyor diye
   çıkarılamaz; kalıpta, produce hedefinde, sohbet açılışında ya da konuşma
   başlığında yaşıyor olabilir. A1'de `umtauschen` 15, `der Kanal` 16 yerde
   geçiyordu. Çıkarmadan önce konuşmanın TÜM metninde ara.
...
3. **Büyük harf isim/fiil ayırır.** `leben` (yaşamak) ile `Leben` (hayat)
   ...
   diğerini yutar — A1'de bir konuşma "hayat" yerine "yaşamak" öğretir hâle
   geldi. Önce tam eşleşme, sonra küçük harf.
4. **Aynı turda yineleme.** Ön denetimin yalnız ÖNCEKİ konuşmalara bakarsa, o
   çalıştırmada eklediğin iki kelime birbirini görmez.
5. **Gloss'u oku.** `die Eins` havuzda "en yüksek not" (Alman okul notu)
   demek, "bir" değil; sayı konuşmasına koymak yanlıştı. Doğrulayıcı bunu
...
reddedilmiyor · konuşma başlığı ve sohbet açılışı yinelenmiyor.

Komutlar: `npx tsc --noEmit` (kök ve `mobile/`), `npm run check:conversations`,
