...
| Kulak kontrolü kararları (2026-09-24) | Canlı `tts_reviews` tablosu (schema `ttsReviews`, `npm run tts:reviews -- import|export [--ok]`). Karar SESE bağlı: `file` yayındaki içerik adresli m4a adı; ses yeniden üretilirse yeniden dinlenir, onaylı ses bir daha sorulmaz. Samet'in notları da burada (sesletim düzeltmelerinin kaynağı). tts-test karar senkronu yazar; paket kurarken ve yeniden üretime aday seçerken `export --ok` ile onaylıları dışarıda bırakır. Yeni bir Whisper taraması da önce buna bakmalı. |
