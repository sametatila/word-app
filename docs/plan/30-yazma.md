...
**Durum (2026-08-26).** Adım 1–3 bitti, 4 pilot (üç A1 egzersizi). `types.ts`: `SkillQuestion.kind` + `accept[]`/`items[]`; `WritingTask` `reply`/`form`/`rewrite`/`summary`. `quiz.tsx`: `ChoiceInput`, `WrittenInput` (gapfill/short_answer/dictation; dikte "Cümleyi dinle" TTS; `written()` toleranslı kabul), `OrderInput` (dokun-takas). `writing-player.tsx`: `FormTask`, `RewriteTask`, `asFree` adaptörü (reply/summary → serbest yazma oynatıcısı, başlık göreve göre). `scripts/check-content.ts` yeni türleri doğruluyor; `data/content/SPEC.md` güncel; `scripts/audit-skills.ts` tip düzeltmesi. İçerik: a1-r1 (+gapfill, +short_answer), a1-l1 (+dictation, +order), a1-w1 (+form, +rewrite); `npm run db:seed:skills` ile üretime yüklendi. Kanıt: `reports/shots/wp31-{reading-gapfill,reading-short,listening-dictation,listening-order,writing-form,writing-rewrite}.png`. Kalan: her egzersize ≥2 yazılı soru ve her seviyede 8 yazma egzersizine yeni tür (WP-72 içerik), B1+ `summary` örnekleri.
