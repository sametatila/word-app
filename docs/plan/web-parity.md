...
| `OnboardingScreen` | `/setup` | **farklı** | Mobil: 4 adımlı sihirbaz (hoş geldin → kurs → başlangıç → günlük hedef) + **ilk 5 kelime** turu. Web `/setup`: tek sayfalık kurs/seviye formu |
| `FirstPracticeScreen` | — | **yok** | İlk kelimeler turu (kayıt öncesi) |
...
1. `/setup` çok adımlı sihirbaza dönüşür (hoş geldin → kurs → başlangıç → günlük hedef).
2. İlk kelimeler turu (`FirstPractice`) web'e gelir.
