...
- **Mod 2 — Immersion:** dili özümseme + üretim. Mevcut "Konuşmalar"in genişletilmiş,
...
- **Konuşma** = anlatım adımları (`say` + `produce`=kendi cümleni kur + `truefalse`) + chat.
  Zaten üretim/immersion birimi. (`lib/conversations/types.ts`)
- Her CEFR seviyesi → **10 tematik modül × 10 konuşma = 100/seviye** (de KURULU: 500 konuşma).
...
- **Modül sınavı** + `moduleClears` → gating altyapısı hazır (`/boss/[level]/[module]`).
...
- **KALIR (TEK iskelet):** yalnız `conversation` içeriği. **Her şey conversationların üzerine inşa edilir** —
  conversation, ünitenin iskeletini ve temasını belirler; diğer item tipleri ona göre türetilir.
...
- **`userConversations` KALIR** — konuşma (conversation item) ilerlemesi + SRS `dueAt` dokunulmaz.
...
4. **Kaldırma:** ✅ TAMAM (kod). `/skills` UI rotaları (page/[id]/loading) + `skills-hub`
   ...
   oynatıcılar, `/api/skills` (ilerleme yazımı), `SkillId` TİPİ — chat/exam/proficiency/
